...
**Week4:** 
- We made the duck idea clearer and started to formulated what the duck has to be able to do. 
...
- I changed several copper cables to flexible cables, so that they are less likely to break and attached them to housings so they fit to the pins of our components. I twisted and tinned the other side of the cable to make them fit onto a breadboard. ([proof](creatingHousingsForFlexibleCables.md))
...
- Since the bolts that held the connector onto the motor were not durable enough, we decided to replace them with metal bolts from the workshop. Therefore I cut them in size and filed the edges to make them go in easier. ([proof](customBolts.md))
...
- I drilled holes in the eyes of the duck to replace them with two red LED's to connect them to a PWM signal and filed down connectors of our breadboard to make it fit into our boxes. ([proof](duckEyes.md))
...
- I tested the step-down-converter for the first time, which worked perfectly and made it way clearer for me on how to implement it into the whole application. We now needed a step-down-converter, since we decided for a second power circuit just for all the components apart from the motors. Therefore we decided to power the LiLiGo trough pen-lite batteries, which needed to go down in voltage.
...
- Since we still had the problem that cables broke or got pulled out while assembling the duck we decided to solder all the previously tinned ends of the flexible cables to pins that fit into the breadboard and turned out more durable. To make sure no short circuits appear we also decided to use heat-shrinks. ([proof](solderingPins.md))
...
2. I want to test out subsystems of the project as prototypes earlier to later have the knowledge to correct all the mistakes I made developing the prototypes in the final assemble.
...
- Me and my team asked Mr. Peters if he has a 12V battery for us and this surprisingly turned out with us deciding against a 12V motor and battery. So it turned out to be more an investigation than just picking up a battery, because of Mr. Peters suggestions. More about that can be seen in [week 6's feedback](/doc/feedbackWeek6.md).


**Week7:**
- It turned out that some components do not fit together, so I had to put some more research in and decided for a [sound-module](https://www.tinytronics.nl/en/audio/audio-sources/open-smart-mp3-module-with-speaker) that makes sure that the speaker, SD-card reader and speaker driver work together.
...
- We probably went 7 or more times after that to the green-tech lab for printing something. I will not put it in every separate week, but it is targeted interaction and the fact that it happened is important for my reflection. 
...
- I informed myself about which saws are for metal asking an Industrial Design Student from the workshop. ([proof](customBolts.md))
...
**What did not work well/ What would I do differently next time?**
...
Even though I chose Design in hardware interfacing as a focus, I did not focus on that accordingly and could have dived deeper into modelling. However throughout the project I did do a lot into the direction of Advice instead, and if I would have picked Advice and not Design, it would have worked for sure.

**What did work well/ What do I want to further improve?**
...
I also believe my drive to put effort into the project and contribute to a final goal was very good.
I generally believe that I am a reliable person and also was one in this project.
